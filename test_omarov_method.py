#!/usr/bin/env python3
"""test_omarov_method.py — Omarov pencereleme yöntemi vs CO=True karşılaştırması.

SORU:
  Omarov σ² bulurken biz CO=True ile σ¹ buluyoruz — neden?
  Yöntem artefaktı mı, yoksa farklı bir fizik mi?

İKİ YÖNTEM:
  A) Omarov: CO=False, t2=T, T'yi N_WIN pencereye böl, her pencerede <Sy> al,
             N_WIN noktaya doğrusal fit → eğim
  B) CO=True: kapalı yörünge üzerinde fırlat, t2=T, tüm noktalara fit → eğim

HER İKİ YÖNTEMDE σ TARAMASI:
  σ ∈ {2, 5, 10, 20, 40} μm, 5 tohum → α_omarov vs α_cotrue

ÇIKTI: test_omarov_method.json, test_omarov_method.png, konsol tablosu
"""
...
def _run_one(task):
    ...
    hist_a, poin_a, pt_a = integrate_particle(
        ...
        return_steps=RET_STEP, quad_dy=quad_dy)
...
def main():
    ...
    print("=" * 72)
    print(f"  OMAROV (pencereleme, N={N_WIN}) vs CO=True karşılaştırması")
    ...
    results = []
    json_path = "test_omarov_method.json"
    ...
    plt.suptitle(f"Omarov pencereleme vs CO=True  |  t2={T2*1e3:.0f} ms, "
                 f"N_win={N_WIN}", fontsize=12)
    plt.tight_layout()
    plt.savefig("test_omarov_method.png", dpi=150)
    print("Figür: test_omarov_method.png")
