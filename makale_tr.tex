...
%==============================================================
\section{R-LS ile sahte-EDM bastırımının doğrudan gösterimi}
\label{sec:bastirimi}

Monte Carlo sistematik analizi $N=200$ gerçekleşme üzerinden \textit{istatistiksel}
doğruluk verirken, bu bölümde yöntemin etkinliği \textit{tek tek gerçekleşmeler}
üzerinden somutlaştırılmaktadır: beş bağımsız rastgele senaryoda sahte-EDM
sinyalinin ne ölçüde bastırıldığı adım adım gösterilmektedir.

\subsection{Senaryo ve hesap adımları}

Her rastgele gerçekleme (tohum) için şu adımlar izlenir:
\begin{enumerate}
  \item \textbf{Hizalama deseni:} Sabit hedef bileşenler ($k=1$: $30\,\mu$m,
    $k=2$: $10\,\mu$m, $k=3$: $25\,\mu$m) artı tohuma özgü rastgele kontaminant
    modlar ($k=4\ldots10$, $100$--$300\,\mu$m rastgele genlik ve faz) kullanılarak
    gerçek kuadrupol kaçıklık vektörü $\Delta q\in\mathbb{R}^{48}$ oluşturulur.
  \item \textbf{Yörünge ve BPM gürültüsü:} Yörünge $\mathbf{y}=R\,\Delta q$ hesaplanır;
    üzerine her tohumda bağımsız Gaussian BPM ofseti
    ($\sigma_b=\SI{50}{\micro\meter}$) eklenir.
  \item \textbf{R-LS (CLEAN) geri çatımı:} Aday mod kümesi $k=1\ldots10$ ile
    CLEAN algoritması çalıştırılır; $k=2$ tahmin katsayısı $\hat{a}_{k=2}$
    elde edilir.
  \item \textbf{Sahte-EDM karşılaştırması:}
    \begin{align}
      |dS_y/dt|_{\text{önce}} &= C\cdot A_{k=2}^{\text{gerçek}}
        = C\cdot\SI{10}{\micro\meter}, \notag \\
      |dS_y/dt|_{\text{sonra}} &= C\cdot\|\mathbf{a}_{k=2}^{\text{gerçek}} - \hat{a}_{k=2}\|,
      \label{eq:supp}
    \end{align}
    burada $C = \SI{1{,}42e-4}{\radian\per\second\per\meter}$ mod taramasından
    elde edilen sahte-EDM ölçek faktörüdür ($A_{k=2}=\SI{10}{\micro\meter}$ için
    $dS_y/dt=\SI{1{,}42e-9}{\radian\per\second}$; bkz. Şekil~\ref{fig:falseedm}).
\end{enumerate}

\subsection{Beş gerçekleme sonuçları}

Şekil~\ref{fig:supp}'de görüldüğü gibi, tüm beş gerçeklemede CLEAN algoritması
$k=2$ bileşenini $\SI{10}{\micro\meter}$'ye yakın tahmin etmekte; artık
genlik $\|\mathbf{a}_{k=2}^{\text{gerçek}}-\hat{a}_{k=2}\|$ ise
$0{,}01$--$\SI{0{,}60}{\micro\meter}$ aralığında kalmaktadır.
Bu değer, $\sigma_b/\|M_{k=2}\|=50/167\approx\SI{0{,}30}{\micro\meter}$
eşleşmiş-filtre teorik alt sınırıyla (BPM-ofset sınırlı bölge)
uyum içindedir.

Beş gerçekleme için $k=2$ tahmin hatası: ortalama $\approx2\%$, standart
sapma $\approx2\%$ (Monte Carlo median değeriyle tutarlı, Bölüm~\ref{sec:hatalar}).
Sahte-EDM bastırım oranları $17\times$--$740\times$ arasında değişmekte;
en az elverişli gerçeklemede bile $17\times$ bastırım sağlanmaktadır.
Medyan bastırım $\approx59\times$'tir.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{fig_8_rls_suppression.png}
\caption{R-LS (CLEAN) ile sahte-EDM bastırımının beş rastgele gerçekleme
için gösterimi. Hizalama deseni: $k=2$ hedef $\SI{10}{\micro\meter}$ (sabit),
$k=4\ldots10$ kontaminant $100$--$300\,\mu$m, BPM ofseti $\sigma_b=\SI{50}{\micro\meter}$.
\textbf{Sol panel:} gerçek $k=2$ genliği (\textcolor{blue}{mavi}),
CLEAN tahmini (\textcolor{orange}{turuncu}),
artık $|$gerçek$-$tahmin$|$ (\textcolor{green}{yeşil}).
Tahmin hatası her çubuğun üzerinde yüzde olarak verilmiştir.
\textbf{Sağ panel:} düzeltme öncesi/sonrası $|dS_y/dt|$ karşılaştırması;
sayısal etiketler bastırım oranını göstermektedir.
Medyan bastırım $\approx59\times$; en kötü durum $17\times$.}
\label{fig:supp}
\end{figure}

\subsection{Fiziksel yorum}

Bastırım oranının tohuma göre farklılaşması iki etkiden kaynaklanmaktadır:
(i)~BPM ofsetinin $k=2$ yörünge desenine projeksiyon genliği ($\|\mathbf{b}\cdot\hat{m}_{k=2}\|$)
ve (ii)~kontaminant modların CLEAN iterasyonunda $k=2$ artığına bıraktığı
sızıntı payı. Bu iki etki rastgele fazlarla toplandığından güçlü bir
girişim (interferans) yaşanabilir: tohum 202'de neredeyse tam giderme ($740\times$),
tohum 505'te ise kısmi toplamlanma ($17\times$) gözlemlenmektedir.
Her iki uç durum da $\sigma_b/\|M_{k=2}\|\approx\SI{0{,}30}{\micro\meter}$
BPM-ofset sınırı ile tutarlıdır.

Bu gösterim, Bölüm~\ref{sec:hatalar}'daki $N=200$ Monte Carlo istatistiğini
bireysel gerçeklemeler düzeyinde somutlaştırmakta ve yöntemin hem doğruluğunu
hem de gerçekçi kötü-durum davranışını açıkça ortaya koymaktadır.
