...
The link between the misalignment split and the false EDM can be written in one
line, and it organizes the entire paper.  Each quadrupole offset $v_j$ gives the
beam a dipole kick, so the closed orbit is linear in the offsets,
\begin{equation}
  x_{\rm CO} = R_x\,v_x, \qquad y_{\rm CO} = R_y\,v_y,
  \label{eq:orbit}
\end{equation}
with $R$ the response matrix of Eq.~(\ref{eq:R}).  The geometric phase, to leading
(second) order in the orbit, is the \emph{directed area} the spin sweeps --- a
quantity bilinear in the two transverse orbits,
\begin{equation}
  f = \frac{dS_y}{dt} \;\propto\; \sum_j w_j
      \big[x_{\rm CO}(s_j)\,y'_{\rm CO}(s_j) - y_{\rm CO}(s_j)\,x'_{\rm CO}(s_j)\big]
      \;=\; x_{\rm CO}^{\!\top} K\, y_{\rm CO},
  \label{eq:area}
\end{equation}
for an antisymmetric kernel $K$.  Substituting Eq.~(\ref{eq:orbit}) makes $f$ a
bilinear form directly in the misalignments,
\begin{equation}
  f = v_x^{\!\top}\,\big(R_x^{\!\top} K R_y\big)\,v_y \;\equiv\; B(v_x, v_y).
  \label{eq:bilinear}
\end{equation}
The projectors act \emph{within} each plane and never couple the two: split each
plane's misalignment separately into its symmetric and antisymmetric parts,
\begin{equation}
  v_x = v_x^{\rm s} + v_x^{\rm a}, \quad v_y = v_y^{\rm s} + v_y^{\rm a},
  \qquad v^{\rm s} \equiv P_{\rm sym} v,\; v^{\rm a} \equiv P_{\rm anti} v ,
  \label{eq:vparts}
\end{equation}
where the \emph{same} operator $P_{\rm sym}$ is applied independently to $v_x$
and to $v_y$ (it is valid in both planes because QF and QD have opposite gradient
in each, so ``move together'' cancels the kicks in both).  What couples the two
planes is the kernel $B$ of Eq.~(\ref{eq:bilinear}), not the projectors.
Bilinearity then splits $f$ into four channels,
\begin{equation}
  ...
  \label{eq:channels}
\end{equation}
Because every factor $R\,v$ is large when the misalignment is antisymmetric and
$G_k$-suppressed when it is symmetric, the pure-symmetric channel $f_{ss}$ is by
far the smallest, and every channel containing the antisymmetric part is larger.
We confirm this directly with the spin tracker [Fig.~\ref{fig:channels}]: feeding
each part pair $(v_x^{a},v_y^{b})$ separately to the C++ integrator, the
four signed channels reproduce the full false EDM to $0.1\%$
($f/\!\sum = 0.999$--$1.001$ over three seeds), a clean check of
Eq.~(\ref{eq:channels}); the pure-symmetric channel is $\sim\!5$--$30\times$
target while the antisymmetric-bearing channels are $\sim\!100$--$900\times$; and
each channel scales as $\sigma^2$.  The consequence, used throughout
Sec.~\ref{sec:results}, is immediate: \emph{ordinary orbit correction removes the
antisymmetric part of the misalignment and with it the three large channels,
leaving the residual false EDM equal to the small $f_{ss}$ --- the one channel
that only a measurement of the symmetric offsets can reduce.}

\begin{figure}[tb]
\includegraphics[width=\columnwidth]{fig_orbit_channels.png}
\caption{The four false-EDM channels of Eq.~(\ref{eq:channels}), measured with
the C++ spin tracker.  (a) At $\sigma=10~\mu$m the pure-symmetric channel
$f_{ss}$ is the smallest; every channel containing the antisymmetric part is
larger (bars: mean of three seeds; dots: individual seeds --- the per-seed
scatter reflects the bilinear, signed nature of $f$).  (b) Each channel scales
as $\sigma^2$, the signature of the geometric phase.  Orbit correction removes
the antisymmetric part, collapsing all channels but $f_{ss}$.}
\label{fig:channels}
\end{figure}
...
\paragraph{Completing the chain: a final orbit correction.}
The $\sim\!17$--$28\times$ residual of the measured-matrix run is not, in fact,
the symmetric floor: it is dominated by an \emph{antisymmetric} leftover.
Decomposing the third-pass residual with the spin tracker, the symmetric part
contributes only $3.5\times$ target, while the antisymmetric part (a
$\sim\!2~\mu$m leftover from the still-imperfect horizontal centering)
contributes $16\times$, with cross terms making up the rest --- exactly the
channel ordering of Eq.~(\ref{eq:channels}).  The antisymmetric part is
orbit-visible, so it is removed by an ordinary SVD orbit correction, the very
first step of the chain.  Applying one to the BBA residual collapses the false
EDM from $28\times$ to below target ($0.0$--$0.8\times$, single beam), leaving a
residual that scales as $\sigma^2$ with no floor down to the $0.25\times$-residual
point we tested.  The picture is thus complete and consistent: \emph{orbit
correction removes the antisymmetric channels; null-finding BBA reduces the
symmetric channel $f_{ss}$ that orbit correction cannot see; together they reach
target}, where each alone stalls (orbit correction at $62\times$, BBA at
$\sim\!20\times$).  We flag two caveats before reading this as a solved problem:
the sub-target endpoint is a single misalignment seed, and $f$ is a signed
bilinear form whose value scatters seed to seed (Fig.~\ref{fig:channels}), so the
\emph{robust} statement is ``down to the symmetric floor, a few times target,''
with a multi-seed campaign needed to fix the number; and this is the single-beam
false EDM, to which the CW/CCW cancellation of Ref.~\cite{omarov} would add
further margin.
...
Sec.~\ref{sec:inverse}, and it reduces the one channel of
Eq.~(\ref{eq:channels}) --- the symmetric $f_{ss}$ --- that ordinary orbit
correction cannot see.  In clean optics it recovers the symmetric component
outright.  Under realistic $\beta$-beating it solves the vertical plane cleanly
and, once steered with the measured response matrix, the horizontal plane too;
followed by a final orbit correction to clear the antisymmetric leftover, the
single-beam false EDM reaches the target for the seed we tested.  This is a
proof of principle rather than a finished tolerance: the horizontal plane still
trails the vertical, the endpoint carries single-seed bilinear scatter, and the
hardware floor below is not yet modeled.  What is established is the mechanism
--- BBA reaches the symmetric channel, orbit correction the antisymmetric one,
and together they cross a target that each misses alone.
...
and the false-EDM floor drops about nine-fold, from $145\times$ to $\sim\!20\times$
target.  What remains at that level is an \emph{antisymmetric} leftover from the
still-imperfect horizontal centering, not the symmetric residual; being
orbit-visible, it is removed by a final orbit correction, which brings the
single-beam false EDM to target for the seed we tested.  The two operations are
complementary in the language of Eq.~(\ref{eq:channels}): orbit correction
removes the antisymmetric channels, null-finding BBA reduces the symmetric
channel $f_{ss}$ that orbit correction cannot see, and together they cross a
target that each misses alone (orbit correction stalls at $62\times$, BBA at
$\sim\!20\times$).  We therefore present null-finding BBA, in a chain with orbit
correction, as the one route \emph{not} excluded by the walls above --- a proof
of principle that the symmetric floor can be reached --- while cautioning that
the sub-target endpoint is a single seed of a scattering bilinear quantity, that
the horizontal plane still trails the vertical, and that a hardware floor
(magnetic-center motion under modulation) remains to be modeled.
