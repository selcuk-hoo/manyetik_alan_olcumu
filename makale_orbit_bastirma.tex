...
realistic optics errors it works in the vertical plane after the orbit is
first flattened and the procedure iterated, but stalls in the horizontal plane
because our steering model treats the electric deflectors as field-free
drifts, which is accurate vertically but not horizontally.  We present this as
the one promising --- but not yet finished --- route, and conclude that
reaching the target by the other techniques would require the symmetric
misalignment to be held below about $1~\mu$m by construction, a tolerance that
no orbit measurement can verify.
\end{abstract}
...
\paragraph{What the completed run shows.}
We ran the full iteration in the C++ tracker with $1\%$ $\beta$-beating, three
passes (Table~\ref{tab:bba}).  The vertical plane behaves exactly as hoped: the
orbit shrinks $48 \to 6.9~\mu$m, the vertical symmetric residual falls roughly
by half each pass ($3.3 \to 1.7 \to 0.9~\mu$m), and it is heading for the
clean-optics floor.  The horizontal plane does not: its symmetric residual
sticks near $2.4~\mu$m.  Because the false EDM is the product $dx\cdot dy$, the
falling vertical part still pulls it down --- from $356\times$ to $145\times$
target over the three passes --- but the stuck horizontal part sets a floor
well above the goal.

\begin{table}[tb]
\caption{Iterated null-finding BBA under $1\%$ $\beta$-beating (C++ tracking,
three passes).  ``sym'' is the symmetric-component residual of
Eq.~(\ref{eq:projectors}).  The vertical residual falls toward the clean floor;
the horizontal one sticks, and the product $dx\cdot dy$ sets the false-EDM
floor.}
\label{tab:bba}
\begin{ruledtabular}
\begin{tabular}{lcccc}
pass & orbit & sym $dy$ & sym $dx$ & $|f|$ / target \\
     & [$\mu$m] & [$\mu$m] & [$\mu$m] & \\
\hline
raw  & 48   & ---  & ---  & $356\times$ \\
1    & 24.1 & 3.30 & 4.48 & $368\times$ \\
2    & 12.6 & 1.72 & 2.99 & $238\times$ \\
3    & 6.9  & 0.91 & 2.38 & $\bm{145\times}$ \\
\end{tabular}
\end{ruledtabular}
\end{table}

\paragraph{Why vertical works and horizontal stalls.}
This is the honest crux, and it is not a law of physics --- it is a limitation
of the \emph{model we steer with}.  The local bumps are built from the analytic
optics solver of Sec.~\ref{sec:method}, which treats the electric deflectors as
field-free drifts.  In the vertical plane that is nearly exact: the ring's
radial deflector field barely focuses vertically, so the model matches the real
machine, the bumps land where intended, and the vertical BBA reaches its floor.
The horizontal plane is the plane the ring bends in, and there the deflectors do
two things the model omits --- they focus the beam horizontally, and they bend
it, which creates dispersion (a coupling between momentum and position).  With
both missing from the steering model, the bumps are mis-sized, worst at the few
locations where the accumulated model error is largest (near the start of the
lattice), and there the zero-crossing is biased and the correction stalls.  A
one-line check confirms the cause.  Because the model treats the deflectors as
drifts, it predicts \emph{identical} tunes in the two planes,
$Q_x = Q_y = 2.303$.  The real ring matches this in the vertical plane to four
decimals ($0.3032$ measured vs.\ $0.3032$), but its horizontal tune is higher by
$0.052$ in fractional part --- the horizontal focusing of the deflectors, absent
from the model.  Since the deflectors are the \emph{only} element that
distinguishes the two planes, this pins the horizontal stall on the model, not on
physics; even in perfectly clean optics (no $\beta$-beating) the same few
horizontal magnets are already mis-measured while the vertical ones are not.  The
standard fix is exactly
what real machines do --- build the bumps from the \emph{measured} horizontal
response of the ring rather than an idealized model --- and is left for future
work.

\paragraph{Two knobs, one trade-off.}
The iteration has two settings: how aggressively to apply each correction, and a
quality cut that throttles magnets whose zero-crossing is inconsistent across
BPMs.  Applied at full strength with no quality cut, the horizontal plane
diverges (its residual blows up to $\sim\!12~\mu$m).  With under-relaxation and
the quality cut, the divergence is stopped ($2.4~\mu$m), but the throttled
horizontal magnet is simply never corrected, so the false EDM plateaus at
$145\times$ rather than reaching the target.  The quality cut buys
\emph{stability}, not a \emph{solution}: the real fix is the better horizontal
model above, not a smarter iteration.
...
outright; under realistic optics it already solves the vertical plane, and the
horizontal plane is a well-understood, fixable modeling gap rather than a
fundamental limit.  We present it as the promising open route, not as a
finished solution --- the horizontal model and the hardware floor below remain
to be done.
...
residual and falling; false EDM $356\times \to 145\times$ over three passes).
The \emph{horizontal} plane stalls, but for an understood and fixable reason:
our steering model treats the electric deflectors as drifts, which is accurate
vertically but not in the bending (horizontal) plane, where the deflectors focus
and create dispersion.  Building the horizontal bumps from the machine's
measured response --- ordinary BBA practice --- is the natural next step.  We
therefore present null-finding BBA as the one route \emph{not} excluded by the
walls above: promising, partly demonstrated, and not yet finished (the
horizontal model and the hardware floor below remain).
