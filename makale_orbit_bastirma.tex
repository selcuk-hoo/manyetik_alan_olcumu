...
\begin{equation}
  ...
  \label{eq:bilinear}
\end{equation}
Inserting $I = P_{\rm sym} + P_{\rm anti}$ [Eq.~(\ref{eq:projectors})] on both
sides splits $f$ into four channels,
\begin{equation}
  f = \underbrace{B(P_{\rm s}v_x,P_{\rm s}v_y)}_{f_{ss}}
    + \underbrace{B(P_{\rm s}v_x,P_{\rm a}v_y)}_{f_{sa}}
    + \underbrace{B(P_{\rm a}v_x,P_{\rm s}v_y)}_{f_{as}}
    + \underbrace{B(P_{\rm a}v_x,P_{\rm a}v_y)}_{f_{aa}}.
  \label{eq:channels}
\end{equation}
...
each projected pair $(P_a v_x, P_b v_y)$ separately to the C++ integrator, the
