...
**ASIL DÜZELTME DENENDİ — ÖLÇÜLEN (C++) TEPKİ MATRİSİYLE YÖNLENDİRME
(`--rmatrix cpp`, TAMAMLANDI):** Tanı doğrulandığına göre çözüm: BBA tümseklerini
idealize analitik model yerine ölçülen C++ tepki matrisinden (`R_d{x,y}_1.npy`,
`build_response_matrix.py`) kur. Element karşılaştırması sorunun büyüklüğünü
gösterdi:

| düzlem | C++/analitik eleman oranı | yanlış düzeltici seçimi |
|---|---|---|
| dikey (y) | medyan 1.05, dar [1.01,1.07] | 12/47 quad |
| yatay (x) | medyan **−0.05**, saçık [−0.13,0.24] | **36/47 quad** |

Yani analitik yatay matris gerçekle neredeyse ilişkisiz (çoğu ters işaret) →
36/47 quad'da YANLIŞ yönlendirme düzelticisi seçiliyor. Temiz-optik izole testte
(orbit-scale=0) C++ R en kötü quad'ı 15× düzeltti (q45: 0.89→0.06 μm), RMS
0.45→0.16 μm. Tam iterasyon (β-beat 1%, 3 geçiş, ölçülen R):

| geçiş | analitik-R | **C++-R** | sym dx | sym dy | min-kalite x |
|---|---|---|---|---|---|
| ham | 356× | 356× | — | — | — |
| 1 | 368× | **72.8×** | 4.27 | 3.27 | 0.10 |
| 2 | 238× | **16.7×** | 2.90 | 1.70 | 0.18 |
| 3 | 145× | **28.4×** | 2.29 | 0.91 | 0.28 |

- **Tanı DOĞRULANDI:** ölçülen matris analitikten ~9× iyi (en iyi 16.7× vs 145×);
  yatay quad'lar artık gate'lenmiyor (min-kalite x 0.00→0.28, sym dx 4.27→2.29).
  Yatay tıkanmanın MODEL hatası (deflektör=drift) olduğu kanıtlandı — fizik değil.
- **AMA tam çözüm değil:** (1) MONOTON DEĞİL — geçiş 2 (16.7×) geçiş 3'ten (28.4×)
  iyi; bilineer sahte-EDM zıplıyor çünkü sym dx tam çözülmüyor (2.3 μm, dikeyin
  0.9'unun gerisinde). (2) HEDEFE ULAŞMIYOR (en iyi 16× üstünde). Yatay çok
  düzeldi ama dikeyin gerisinde.
- **Kalan yatay açık:** olası nedenler — C++ R temiz latiste ölçüldü ama koşumda
  β-beat var (R hâlâ hafif uyumsuz; gerçekte β-beat'li makinede ölçülmeli);
