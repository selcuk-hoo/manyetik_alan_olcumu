...
\paragraph{Yapısal sınır (kesin).} Drift monitörün lineer modeli
(Eş.~\eqref{eq:lin}) yatay ve dikey düzlemleri \textbf{ayrık} ele alır: $dy$
yalnız dikey, $dx$ yalnız yatay yörüngeyi üretir; birinci derecede çapraz terim
yoktur (skew kuplajı = kuadrupol dönmesi sıfır). Tablo~\ref{tab:t6}'nın $x$ ve
$y$ sütunlarını bağımsız vermesinin sebebi budur: problem iki ayrık
$48\times48$ sistemdir. Dolayısıyla monitör, sahte EDM'i süren \textbf{bilineer
$dx\cdot dy$ büyüklüğünü yapısal olarak temsil edemez, kısıtlayamaz} --- bu,
mod/gözlenebilirlik ayrıntısından bağımsız, kesin bir ifadedir. Kapalı yörünge
tek düzlemin \emph{ortalama} yer değiştirmesini taşır, iki düzlemin
\emph{çarpımını} değil.
