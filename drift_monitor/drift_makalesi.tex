...
arasında lineer (Pearson) korelasyon $0.98$; $\sigma_k/G_k$ oranı tüm spektrumda yalnız
...
\caption{SVD spektrumunun kazanç yasasıyla birleşmesi: her SVD modunun tekil
değeri $\sigma_i$, o modun kick-harmoniğindeki kazanç
$G_k=C/|Q_{\rm eff}^2-k^2|$'ya karşı (log-log). Noktalar $\sigma\propto G_k$
doğrusunu izler (Pearson korelasyon $0.98$, $\sigma\approx1.22\,G_k$); renk modun simetrisi $\chi_i$.
Yüksek-$\sigma$ (iyi gözlenen) modlar antisimetrik ($\chi\!\to\!-1$, kick
tune'a yakın), düşük-$\sigma$ modlar simetrik ($\chi\!\to\!+1$, kick
$k\!\approx\!24$). $G_k$ analitik rezonans iskeleti, $\sigma_i$ $\beta/KL$-ağırlıklı
kesin spektrumudur.}
\label{fig:svdgain}
\end{figure}
