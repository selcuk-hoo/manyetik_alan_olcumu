...
göre $\sim 193$ kat) maruz kalır. Ayrıca sahte EDM'i süren baskın kanal, yatay
ve dikey kaçıklığın \textbf{çarpımına} ($dx\cdot dy$, $\propto\sigma^2$) bağlı
bilineer bir geometrik-faz etkisidir \cite[Fig.~9a]{omarov2022}; lineer ve
düzlem-ayrık olan drift monitör bu çarpımı \textbf{yapısal olarak temsil edemez}.
Bu iki yapısal neden --- düzlem-ayrıklık ve simetrik alt-uzay körlüğü ---
kapalı-yörünge izlemenin antisimetrik hizalama driftini hassasça takip ederken,
sahte-EDM-kritik bilgiyi tanımlı biçimde dış bir gözlemlenebilire (karşı-dönen
demet ayrımı / spin) bıraktığını gösterir. Yöntemin kesin tanımlı bir
...
\subsection{Bunun sahte EDM ile ilişkisi --- ve kesin sınır}
Drift monitörün amacı hizalama driftini izlemekti; ama asıl önemsediğimiz sahte
EDM'dir. İki arasındaki bağı dürüstçe, sınırlarıyla kuruyoruz.

\paragraph{Sahte EDM'in baskın kanalı bir yatay--dikey kuplajıdır.} Sahte
vertikal spin presesyonunu ($dS_y/dt$) süren baskın terim, yatay ($dx$) ve dikey
($dy$) kaçıklığın \textbf{çarpımına} orantılı bir geometrik (Berry) faz
katkısıdır; kaçıklıkla \textbf{kuadratik} ($\propto\sigma^2$) ölçeklenir
\cite[Fig.~9a]{omarov2022}; bizim \S13.7 spin-takip taramamız $\sigma$-üssünü
$2.0$ ölçer. Yalnız-$dy$ katkısı doğrusal ve küçüktür; yalnız-$dx$ ise $S_y$
için sıfırdır. Anahtar sözcük \textbf{çarpım} $dx\cdot dy$'dir: etki iki düzlemin
\emph{birlikte} kaçık olmasını gerektirir.

\paragraph{Yapısal sınır (kesin) --- asıl mesele lineerlik.} Sahte EDM'i süren
büyüklük iki düzlemin \textbf{çarpımıdır} ($dx\cdot dy$); yani misalignment'ta
ikinci dereceden (bilineer). Oysa \textbf{kapalı-yörünge tepkisi misalignment'ta
lineerdir} (Eş.~\eqref{eq:lin}). Kritik nokta düzlem-ayrıklık \emph{değil},
lineerliktir: quad tilt bir skew kuplajı (yatay$\leftrightarrow$dikey çapraz
bloklar) eklese bile tepki \emph{hâlâ lineer bir haritadır} ve hiçbir
$dx\cdot dy$ çarpım terimi üretmez. Çapraz kuplajı açıkça ölçtük
(Tablo~\ref{tab:tilt}): 0.2\,mrad tilt'te kuplaj diyagonalin \%0.33'ü ve drift
kurtarımını hiç değiştirmez --- çünkü mesele kuplajın büyüklüğü değil, gözlenenin
\emph{derecesidir}. \textbf{Lineer bir gözlemlenebilir, bilineer bir büyüklüğü
ilkesel olarak kısıtlayamaz}; dolayısıyla monitör --- kuplajlı ya da değil ---
sahte EDM'i süren $dx\cdot dy$'yi yapısal olarak göremez.
(Tablo~\ref{tab:t6}'nın $x/y$ sütunlarının ayrık olması bu lineerliğin bir
görünümüdür: kapalı yörünge tek düzlemin \emph{ortalama} yer değiştirmesini
taşır, iki düzlemin \emph{çarpımını} değil.)

\paragraph{Düzlem-içi sınır.} Buna ek olarak, her düzlemin kendi içinde monitör
simetrik (yüksek-$k$) alt-uzaya \S4.2--4.3 gereği neredeyse kördür (en kötü modda
...
\paragraph{Açıkça belirtilen sınır (dürüstlük).} Baskın $dx\cdot dy$ kanalının
ağırlığının özellikle \emph{simetrik} alt-uzayda mı yoğunlaştığı ayrı,
desen-bağımlı bir sorudur; spin taramamız bu kanalın seed'e göre $\sim 36\times$
saçıldığını gösterir (\S13.7) ve bu ayrışım tek bir kapalı-yörünge ölçümüyle
çözülemez --- bu da rapor ettiğimiz sınırın bir parçasıdır. Yalnız-$dy$ (baskın
olmayan) kanalda spin simülasyonu, gözlenebilir harmonikler çıkarıldıktan sonra
artık sahte-EDM'in küçük kaldığını gösterir; baskın kanal için kesin nicel
alt-uzay ayrışımı gelecek iştir.

\paragraph{Sonuç.} Kapalı-yörünge drift monitörü sahte-EDM-kritik bilgiyi
\textbf{iki bağımsız yapısal nedenle} kısıtlayamaz: (1) lineer ve düzlem-ayrık
olduğundan baskın kanalı süren bilineer $dx\cdot dy$'yi ilkesel olarak göremez;
...
erişim \textbf{farklı bir gözlemlenebilir} gerektirir --- karşı-dönen (CW/CCW)
demet ayrımı veya doğrudan spin presesyonu \cite{omarov2022}. Drift monitörün
net rolü budur: antisimetrik hizalama driftini ucuza ve sürekli izler,
sahte-EDM-kritik kanalı tanımlı biçimde dış bir gözlemlenebilire bırakır.
...
Daha önemlisi, yöntemin \textbf{temel sınırı} iki bağımsız yapısal nedenle
nicelendi (\S4.4). \emph{Birincisi (kesin):} sahte EDM'i süren baskın kanal,
yatay ve dikey kaçıklığın çarpımına ($dx\cdot dy$) bağlı bilineer bir etkidir
\cite{omarov2022}; lineer ve düzlem-ayrık olan monitör bu çarpımı yapısal olarak
temsil edemez (Tablo~\ref{tab:t6}'da $x$/$y$ sütunlarının bağımsızlığı bunun
göstergesi). \emph{İkincisi:} her düzlem içinde, per-mod SVD analizi monitörün
kör noktasının \%96 simetrik içerikli en kötü koşullanmış modlar olduğunu
gösterir (en kötü modda en iyiye göre $\sim 193$ kat duyarlılık dezavantajı).
Baskın kanalın ağırlığının özellikle simetrik alt-uzayda mı yoğunlaştığı ise
desen-bağımlı, açık bir sorudur (\S4.4). Net sonuç: antisimetrik drift hassasça
izlenir; sahte-EDM-kritik bilgi ise donanım/veriyle kapanmayan yapısal nedenlerle
erişilemez ve farklı bir gözlemlenebilir (karşı-dönen demet ayrımı / spin)
gerektirir \cite{omarov2022}.
