...
\paragraph{SVD spektrumu $G_k$'nın kesin gerçekleşmesidir.} Bu $\sigma_i$'ler
\S4.2'deki kazanç yasasından bağımsız değildir: her SVD modu (yaklaşık) bir
kick-harmoniğine karşılık gelir ve tekil değeri o harmonikteki kazançla
orantılıdır, $\sigma_k\propto G_k$. (Sayısal teyit: mod kick-harmoniği ile $G_k$
arasında lineer (Pearson) korelasyon $0.995$; $\sigma_k/G_k$ oranı tüm spektrumda yalnız
$\sim$1.2--3.4 --- bu, $\sqrt\beta$ ve $KL$ ağırlığından.) Yani $G_k$ rezonans
\textbf{iskeletini} ($k\approx Q$'da güçlü, $k\gg Q$'da zayıf), SVD ise onun
$\beta/KL$-ağırlıklı \textbf{kesin} spektrumunu verir; geri-çatım gürültüsü
doğrudan rezonans paydasıdır, $1/\sigma_k\propto|Q_{\rm eff}^2-k^2|$. Kısaca
$\chi$ (hangi bant), $G_k$ (rezonans) ve $\sigma_i$ (kesin spektrum) tek
hikâyenin parçalarıdır (Şekil~\ref{fig:svdgain}).

\begin{figure}[tb]
\centering
\includegraphics[width=\columnwidth]{fig8_svd_gain.png}
\caption{SVD spektrumunun kazanç yasasıyla birleşmesi; yatay eksen her iki
panelde de modun fiziksel kick harmoniği $k$. \textbf{(a)} Her SVD modunun tekil
değeri $\sigma_i$ (noktalar), kendi $k$'sindeki rezonans kazancı
$G_k=C/|Q_{\rm eff}^2-k^2|$ teorik eğrisine (kesik çizgi, $a=1.22$ ölçeği)
oturur: $\sigma_i\approx a\,G_k$ (Pearson korelasyon $1.00$). Kazanç $k\approx
Q_{\rm eff}\approx2.24$'te tepe yapar, yüksek-$k$'de bastırılır. \textbf{(b)}
Aynı modların simetrisi $\chi_i\in[-1,+1]$: iyi gözlenen düşük-$k$ uç
antisimetrik ($\chi\!\to\!-1$), bastırılan (gürültü yükselten) yüksek-$k$ uç
simetrik ($\chi\!\to\!+1$). $G_k$ analitik rezonans iskeleti, $\sigma_i$ ise
onun $\beta/KL$-ağırlıklı kesin spektrumudur; gürültü tabanı bu bastırılmış
simetrik uçtan gelir.}
\label{fig:svdgain}
\end{figure}
