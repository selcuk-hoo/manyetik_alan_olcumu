...
**$\sim$10 $\mu$m seviyesinde bilinmesi/kontrol edilmesi** gerektiğini göstermiştir —
...
3. **Nicel karakterizasyon (destekleyici).** Yöntemin geçerlilik alanı
   (antisimetrik/korelasyonsuz drift, 6–7 $\mu$m RMS) ve kör noktası (simetrik/koherent,
...
(§3.1) — 10 $\mu$m hedefin çok üstünde.
...
**Simülasyon bunu doğruluyor:** ham $\Delta R^{-1}$ Test 1'de 1865 $\mu$m, Test 6'da
$\sim$3000 $\mu$m verir; Şekil 6 gürültünün $\propto1/\varepsilon$ patladığını
...
Soru: §2.3-(iii)'teki düzenlileştirme, (ii)'nin gürültü patlamasını 10 $\mu$m
...
|---|---|---|---|
| Direct $R^{-1}$ (ofsetsiz) | 3.5 $\mu$m | 0.998 | yalnız *referans*: ofset=0 alındığında gürültü tabanı |
| Ham $\Delta R^{-1}$ (ii) | 1865 $\mu$m | 0.085 | düzenlileştirmesiz iki-gradient: gürültü patlar |
| Tikhonov (iii) | 53 $\mu$m | 0.348 | $\lambda$ ile dengelenmiş; biçim büyük ölçüde kayıp |
| TSVD ($k=3$, iii) | 52 $\mu$m | 0.383 | en iyi 3 mod tutulmuş ($k$ ideal seçilmiş üst-sınır) |

İki ders: (a) **düzenlileştirme gürültüyü ~35× bastırır (1865→~52 $\mu$m) ama
...
$R^{-1}$ (3.5 $\mu$m) burada en iyi görünür ama **operasyonel rakip değildir** —
ofset=0 varsaydığı için gerçek halkada ($\mathbf b\neq0$) 200+ $\mu$m'e fırlar
...
3.48–4.01 $\mu$m (0.5 $\mu$m değişim); dikey sabit (Maxwell). Gerçek halkada LOCO'nun
...
**Senaryo.** $t=0$: 100 $\mu$m RMS hizalama + 50 $\mu$m RMS BPM ofseti kaydedilir.
$t=1..10$: hizalama 10 $\mu$m RMS yavaşça kayar, ofset sabit.

| Yöntem | Düzlem y | Düzlem x |
|---|---|---|
| Mutlak $R^{-1}\mathbf{y}(t)$ | ~197 $\mu$m (ofset baskın) | ~185 $\mu$m |
| **Drift $R^{-1}(\mathbf{y}-\mathbf{y}_0)$** | **6.6 $\mu$m** | **6.5 $\mu$m** |
| İyileşme | 29× | 28× |
...
Aynı veri, aynı bütçe (50 $\mu$m ofset, 1 $\mu$m gürültü, 10 $\mu$m drift, 0.2 mrad
tilt'ler):

| Estimatör | y-RMS | y-korr | x-RMS | x-korr |
|---|---|---|---|---|
| A: analitik $\Delta R$ | 3282 $\mu$m | -0.02 | 3756 $\mu$m | 0.09 |
| **B: drift modu** | **6.25 $\mu$m** | **0.85** | **7.18 $\mu$m** | **0.85** |
| C: sayısal $\Delta R$ | 980 $\mu$m | -0.02 | 1357 $\mu$m | -0.11 |
...
yalnız **%0.33'ü**; drift takip hatası **değişmez** (6.27 $\mu$m). 1 mrad'a kadar
(kuplaj %1.1) hata yine sabit. Yani çapraz kuplaj, drift modunun ölçtüğü *değişime*
$\sim$(kuplaj)$\times$(drift) $\approx 0.1\,\mu$m katkı verir — 6 $\mu$m tabanının
çok altında. **Düzlem-ayrık model gerçekçi quad tilt altında geçerlidir.**

| Quad tilt | $x\!-\!y$ kuplajı $\|R_{yx}\|/\|R_{xx}\|$ | $y$-takip | $x$-takip |
|---|---|---|---|
| 0 | %0.00 | 6.27 $\mu$m | 5.95 $\mu$m |
| **0.2 mrad** | **%0.33** | **6.27 $\mu$m** | **5.95 $\mu$m** |
| 1 mrad | %1.08 | 6.26 $\mu$m | 5.95 $\mu$m |
...
| β-beating ($\varepsilon_\beta$) | y [$\mu$m] | x [$\mu$m] | Yorum |
|---|---|---|---|
| 0% (mükemmel) | 5.98 | 5.88 | gürültü tabanı |
| 0.5% | 5.94 | 5.91 | önemsiz |
| **1% (LOCO-gerçekçi)** | **6.08** | **6.09** | **hedef altında** |
| 2% | 6.48 | 6.58 | <7 $\mu$m |
| 5% | 8.57 | 9.13 | <10 $\mu$m güvenli marj |
| 10% | 13.00 | 14.51 | sınır aşılır |

LOCO sonrası tipik $\sim\%1$ β-beating'de taban yalnızca 0.1 $\mu$m artar
(5.98→6.08 $\mu$m); hedefe 3.9 $\mu$m marj kalır. Yöntem, standart-kalite LOCO'su olan
...
(focal) hata RMS'i 0, %2, %5 için drift takip hatası 6.17, 6.17, 6.25 $\mu$m
...
Tipik %2 kazanç hatasında taban yalnız 0.6 $\mu$m artar (5.7→6.4 $\mu$m); %5'e kadar
...
| Test | Soru | Ana sayı |
|---|---|---|
| 1 | Düzenlileştirme $\Delta R$'yi kurtarır mı? | 1865→52 $\mu$m, ama direct 3.5 $\mu$m |
| 2 | Düzenlileştirme nasıl çuvallıyor? | 48 modun 43–45'i siliniyor |
| 3 | Yatay modelde ters-suç? | $\pm10\%$ → <0.5 $\mu$m |
| 4 | Drift modu ofseti tolere eder mi? | 197 $\mu$m → 6.6 $\mu$m (29×) |
| 5 | Ofset kayarsa? | $<2\,\mu$m/epoch'a kadar üstün |
| 6 | Aday yöntemler yan yana? | $\Delta R$: 1000–3700 $\mu$m, drift: 6–7 $\mu$m |
| 8 | β-beating? (gerçek gradyan/focal hatası) | %1→6.1 $\mu$m; %5→8.6 $\mu$m |
| (tilt) | quad tilt / x–y kuplajı? | 0.2 mrad: kuplaj %0.33, takip değişmez |
| 9 | BPM kazanç hatası? | %2→6.4 $\mu$m; %5→8.6 $\mu$m; hedef altında |

---

## 4. Gözlenebilirlik Sınırı

§3'te drift modunun bir **gürültü tabanına** ($\sim6$–7 $\mu$m, Test 4) oturduğunu
gördük: 1 $\mu$m BPM gürültüsü $R^{-1}$'den geçerken $\sqrt2\,\sigma_\eta\|R^{-1}\|$
...
çözüldüğünü belirler. Somut olarak, 6–7 $\mu$m'lik tabana en büyük katkı, $R$'nin en
...
### 4.5 Kör noktanın sahte-EDM bütçesiyle ilişkisi (Berry Haritası)
...
ya da daha fazla veriyle kapanmaz.

Bu kör noktanın **sistematik bütçe** için ne kadar önemli olduğu kritik bir
sorudur. pEDM bağlamında ilgili sistematik, kaçıklığın ürettiği sahte
...
Sahte EDM'nin kapalı yörüngeden *doğrudan* okunup okunamayacağını incelemek
için makine öğrenmesi (Ridge regresyonu) destekli bir Berry fonksiyoneli
($\sum w_i x_i y_i$) geliştirilmiştir. Şekil~\ref{fig:berrymap}, analitik SVD
modlarının gözlenebilirliği ($\sigma_i$) ile bu modların ürettiği sahte-EDM
katkısını ($|f_{\text{pred}}|$) karşılaştırmaktadır.

Harita (Şekil~\ref{fig:berrymap}) iki zıt küme gösterir:
1. **İzle (Görünür + EDM sürücü):** Antisimetrik modlar kapalı yörünge monitöründe
   güçlü sinyal verir ($\sigma \gtrsim 1$) ve sahte EDM üretir. Orbit düzeltmesi
   bu modları tespit edip telafi etmekte başarılıdır.
2. **Kör Artık (Görünmez + EDM sürücü):** Simetrik modlar sahte EDM üretmesine
   rağmen orbit monitöründe neredeyse görünmezdir ($\sigma \ll 1$). Bu modlar,
   standart yörünge düzeltmesi sonrası geriye kalan "irredüsibl (indirgenemez)
   artık"ı oluşturur.

Sonuç: Tanının kör alt-uzayı tam da sahte EDM'yi süren simetrik artıkla
çakışır. Orbit-tabanlı izleme onu bastıramaz; bu alt-uzaya erişim ilkesel
olarak farklı bir gözlemlenebilir (ör. karşı-dönen CW/CCW demet ayrımı veya
doğrudan spin presesyonu [Omarov 2022]) gerektirir. Yörünge-tabanlı izleme
ise ancak antisimetrik/gözlenebilir driftleri bastırmak için değerli bir
ilk aşama olarak hizmet eder.
...
izler (geçerlilik alanı; §3'te 50 $\mu$m ofset + %1 β-beating altında 6–7 $\mu$m RMS,
...
BPM kazanç hataları (Test 9, %2→6.4 $\mu$m), kuadrupol tilt'inin yarattığı x–y skew
...
rekonstrüksiyon hatası epoch'a karşı (log ölçek). 50 $\mu$m BPM ofseti mutlak
rekonstrüksiyonu ~200 $\mu$m'de boğarken drift modu 6–7 $\mu$m RMS'te izler. (§3.4)
...
Kesik kırmızı çizgi 10 $\mu$m hedef, noktalı gri çizgi LOCO-gerçekçi %1 seviyesi.
%1'de hata 6.1 $\mu$m; %5'e kadar hedef altında. (§3.7)
...
<a id="fig:berrymap"></a>
**ŞEKİL 8.**
![Mod-bazlı gözlenebilirlik ve sahte EDM haritası](berry_observability_map.png)
 Mod-bazlı gözlenebilirlik ($\sigma_i$)
ile sahte-EDM katkısının ($|f_{\text{pred}}|$) dört-çeyrek haritası. Antisimetrik
modlar (kırmızı) yüksek gözlenebilirliğe sahipken, simetrik modlar (mavi kare)
kör alt-uzaya düşer ve yörünge düzeltmesi sonrasında sahte EDM'yi süren
"irredüsibl" artığı oluşturur. (§4.5)
...
10 $\mu$m hedef, noktalı gri tipik %2 seviyesi. Sabit ofset gain'den bağımsız iptal
