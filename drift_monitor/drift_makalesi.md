...
ölçüm konfigürasyonu altında gürültü sınırında bırakır. Makalenin merkezi
tezi tam da budur: ölçülen 6 μm sayısı tek başına değil, **kapalı-yörünge
gözleminin yalnızca belirli bir alt-uzayı çözebildiği ve bu alt-uzay
ayrımının SVD aracılığıyla fiziksel sistematik kanallarla ilişkilendirilebildiği**
sonucudur. Böylece kapalı-yörünge izlemenin kesin tanımlı bir **geçerlilik
alanı** ve kesin tanımlı bir **kör noktası** olduğu gösterilir.
...
saklanır). Gürültü için bu çalışmada $\sigma_\eta = 1\,\mu$m alınır; bu sayı
ham BPM elektronik gürültüsü değil, çok-tur ortalaması sonrası **etkin yörünge
belirleme belirsizliği** olarak yorumlanmalıdır (modern depolama halkalarında
tur-ortalamalı çözünürlük tipik olarak bu mertebededir). Yatay düzlemde elektrostatik ark deflektörlerinin katkısı için
...
gösterdi. Bu, kapalı-yörünge yöntemlerinin sahte-EDM-kritik bilgiyi bu ölçüm
