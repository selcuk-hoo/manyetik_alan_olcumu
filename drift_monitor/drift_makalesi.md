# Dondurulmuş-Spin Proton EDM Halkasında Kapalı-Yörünge Tabanlı Hizalama Drift İzleme: Yetenekler ve Gözlenebilirlik Sınırı
...
Yöntem, kalibrasyon anındaki BPM okumasını referans alarak
...
RMS hassasiyetle çalışır (mutlak rekonstrüksiyona göre $\sim 29\times$
...
**simetrik** alt-uzay; hücre içi QF/QD aynı yönde) en büyük gürültü
büyütmesine ($\sim 193\times$) maruz kalır. Bu simetrik alt-uzay, sahte EDM'yi
süren $dx\cdot dy$ geometrik-faz kanalıyla ($\propto\sigma^2$) çakışır
[Omarov 2022, Fig. 9a]; dolayısıyla kapalı-yörünge izleme antisimetrik
hizalama driftini hassasça takip ederken, sahte-EDM-kritik simetrik kanalı
yapısal olarak gürültü sınırında bırakır. Sonuç, kapalı-yörünge izlemenin
kesin tanımlı bir **geçerlilik alanı** ve kesin tanımlı bir **kör noktası**
olduğunu gösterir.
...
2. **Yapısal arka plan (destekleyici).** İki-ölçümlü, tam-ofset-iptal eden
   lineer estimatör sınıfı için bir alt sınır türetilir
...
3. **Temel sınır (özgün omurga).** Per-mod SVD analiziyle, kapalı-yörünge
   izlemenin kör noktasının **simetrik alt-uzay** olduğu ve bunun sahte-EDM'yi
   süren $dx\cdot dy$ kanalıyla çakıştığı gösterilir (§4). Bu, kapalı-yörünge
   yöntemlerinin sahte-EDM-kritik bilgiyi ilkesel olarak neden sağlayamadığının
   nicel bir ifadesidir.
...
saklanır). Yatay düzlemde elektrostatik ark deflektörlerinin katkısı için
...
### 2.4 Ofset–gürültü dualite teoremi (destekleyici)
...
Drift modu $\mathbf{b}(t)\approx\mathbf{b}_0$ varsayar. Ofset $\sim\mu$m/epoch
kayarsa bozulur; alternatif olarak per-epoch iki-gradient + 30-epoch ortalama.
Geçiş $\sim 2\,\mu$m/epoch'ta. Termal-stabil bir salonda modern BPM
elektroniği bunun çok altındadır ($\sim 0.1\,\mu$m/°C); drift modu baskın
kalır.
...
Azimutal harmonik $k$'lı bir kick desenine kapalı-yörünge tepkisi, tune'da
rezonant bir kazançla ölçeklenir:
...
en kötü 8 mod ortalama **%96** simetrik. En kötü modun gürültü büyütmesi en
iyiye göre $\sim 193\times$'tir. Yani $\kappa(R)=193$'ün tamamı, simetrik
alt-uzayın kötü koşullanmasından gelir. (`drift_monitor/permode2.py`)
...
Bu kuadratik kanalı süren misalignment deseni tam olarak **simetrik
alt-uzaydadır**. Bizim simülasyonlarımız bu mekanizmayı bağımsız olarak
doğrular (çok-tohum RMS taramasında $\sigma$-üssü $\approx 2.0$).

Sonuç birleştiğinde: kapalı-yörünge izleme, sahte-EDM-kritik simetrik
deseni $\sim 193\times$ gürültü büyütmesiyle "görür". Mevcut 1 μm BPM
gürültüsünde bu kanalda etkin hassasiyet $\sim 193\,\mu$m mertebesine
düşer — drift modunun $\sim 6\,\mu$m'lik RMS tabanına bu simetrik bileşen
karışır, ama **sahte-EDM açısından kritik kısım iki mertebe daha az
hassasiyetle** kestirilebilir. Bu, daha iyi BPM donanımı veya daha fazla veri
yığılmasıyla **aşılamayan** yapısal bir sınırdır; doğrudan simetrik
alt-uzayın yüksek-$k$ doğasının ($G_k\propto 1/|Q^2-k^2|$, $k\approx 24$) bir
sonucudur. Simetrik kanala doğrudan duyarlı bir gözlemlenebilir (ör.
karşı-dönen demet ayrımı veya spin presesyonu) bu sınırın dışındadır
[Omarov 2022].
...
koşullanmış modlar olduğunu ($193\times$ gürültü büyütmesi) ve bu simetrik
alt-uzayın sahte EDM'yi süren $dx\cdot dy$ geometrik-faz kanalıyla çakıştığını
gösterdi. Bu, kapalı-yörünge yöntemlerinin sahte-EDM-kritik bilgiyi neden
ilkesel olarak sağlayamadığının nicel bir ifadesidir: antisimetrik drift
hassasça izlenir, simetrik (kritik) kanal yapısal olarak gürültü sınırındadır
ve bu sınır donanım veya istatistikle aşılamaz. Bu kanala erişim, farklı bir
gözlemlenebilir gerektirir [Omarov 2022].
