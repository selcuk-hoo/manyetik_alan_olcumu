...
Koşul nettir ve dürüstçe ifade edilir: kalan sahte-EDM hedefin altına \emph{ancak}
optik-model ($\beta$-beating) $\varepsilon \lesssim \%0.5$--$1$ düzeyinde
bilindiğinde iner (Tablo~\ref{tab:linchpin}). Bu sınır istatistiksel değildir
(istatistiksel taban $\sim$24~nm $\to 4\times10^{-11}$ rad/s) ve no-go'nun
indirgenemez simetrik tabanı da değildir; LOCO ile ulaşılabilir bir
\emph{kalibrasyon} hedefidir. Yöntemin asıl katkısı tekniksel yenilik değil
