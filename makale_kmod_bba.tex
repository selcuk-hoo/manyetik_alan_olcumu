...
Sahte-EDM'in kapalı yörüngenin bir \emph{bilineer fonksiyoneli} olduğunu, halka
boyunca $N$ noktada örneklenen yörüngeden $f \approx \sum_i w_i x_i y_i$ formuyla
gösteriyoruz; $w_i$ ağırlıkları veriden (ridge regresyon) öğrenilir. 64 senaryoda
\emph{görmediği} veride çapraz-doğrulama (LOO): üniform $\langle xy\rangle$ yalnız
$\sim$\%30, ağırlıklı $\sum_i w_i x_i y_i$ ise \textbf{\%88} açıklar
($R^2=0.88$, permütasyon-null maks $\sim$0.2 $\to$ ezber değil). Öğrenilen $w_i$
profili farklı $N$/$\lambda$'da kararlıdır ve üniform değildir
(Şekil~\ref{fig:berry}) --- bu, \cite{omarov2022} Şekil 9(a)'daki $\sigma^2$
geometrik fazın yörünge-uzayındaki nicel karşılığıdır.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{berry_data/berry_weights.png}
\caption{Öğrenilen Berry ağırlığı $w_i$ halka konumuna ($s/C$) karşı; farklı
örnekleme $N$ ve düzenlileştirme $\lambda$ için kararlı ve üniform değil (basit
$\langle xy\rangle$ yetersiz). Kesik çizgi: $|x|$ orbit-genlik profili (ölçekli).}
\label{fig:berry}
\end{figure}
