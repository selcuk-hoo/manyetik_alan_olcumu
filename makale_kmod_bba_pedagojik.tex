...
doğrulanmış bir tahmin ediciyle ($\sigma^2$ testinde üs $p=2.00$) gösteriyor
(yörüngeden $f$'i öngören kapalı fonksiyonel açık problem olarak kalıyor);
(ii) hizalama hatalarını antisimetrik
...
\caption{Sahte-EDM hizalama RMS'i $\sigma$ ile \emph{kuadratik} ölçeklenir:
log-log eğim $p=2.00$ ($\sigma=2.5,5,10\,\mu$m, 4D-CO + model-fit estimator).
Bu üs, geometrik (Berry) fazın parmak izidir --- doğrusal bir kaçak olsaydı eğim
1'e kayardı.}
\label{fig:sigma}
\end{figure}

\textbf{Sahte EDM yörüngenin neresinden gelir? (açık problem).} Sahte EDM bir
kapalı-yörünge fonksiyonelidir, ama \emph{kapalı formunu} (örneğin basit bir
$f \approx \sum_i w_i x_i y_i$ bilineer çekirdeğini) bu çalışmada güvenilir biçimde
belirleyemedik; açık bırakıyoruz.

\Sezgi{Neden açık? Halkayı $N$ noktada örnekleyip $f \approx \sum_i w_i x_i y_i$
fitlersek, serbest (latise bakmayan) ağırlıklarla görmediği veride
LOO-$R^2\!\approx\!0.88$ alırız --- cazip görünür. \emph{Ama bu bir tuzak:} halka
24 özdeş FODO hücresinden oluşur, dolayısıyla fiziksel duyarlılık çekirdeği
\textbf{24-periyodik} olmak zorundadır. 24-periyodikliği zorladığımızda
(tek-hücre çekirdek) LOO-$R^2$ \textbf{0.10}'a düşer (öngörüsüz); serbest fitin
gücü ise $k\!=\!1$--$6$ harmoniklerinde ve uç-noktalarda toplanır
($w_{s=0}\!=\!-1$). Yani $0.88$, latis simetrisini ihlal eden örneğe-özgü bir
\emph{aşırı-uyumdur}, fiziksel çekirdek değil. Dürüst kayıt:
\texttt{orbit\_ileri\_olcum.md}.}

Bu \emph{yöntemimizi etkilemez}: AC-BBA, sahte-EDM'i yörüngeden \emph{öngörmeye}
çalışmaz --- demet-quad ofsetlerini doğrudan ölçer (§\ref{sec:method}). Mekanizma
için güvenilir nicel dayanak, basit bir fonksiyonel değil, \textbf{$\sigma^2$
ölçeklemesidir} ($p=2.00$, Şekil~\ref{fig:sigma}; \cite{omarov2022} Şekil 9(a)).
...
uzantısıyla ölçülmelidir. \textbf{CW/CCW + quad-flip bunu kurtarmaz} (20-seed ensemble): idealize latiste
CCW ile quad-flip \emph{özdeştir} (dejenerasyon, oran 1.00) --- flip bağımsız bir
iptal knob'u vermez; ve tilt'in EDM-kanalına katkısı demet-yönü ters-çevirmesi
altında \emph{dominant tek} (odd) bileşene sahiptir (ODD/EVEN $\approx 1.3$--$1.9$),
yani CW/CCW farkı onu söndüremez --- kalan katkı hedefin $\sim$8--57$\times$
üstündedir. Tilt, $\beta$-beating'den ayrı, açıkça belirtilen bir bütçe kalemidir
ve mekanik roll ($\lesssim$0.3~mrad) veya skew-BBA ile kontrol edilmelidir.
Doğrudan ölçtük (6-seed $\psi$-taraması, 0.3~$\mu$m kalan misalignment üstünde):
$\psi=0.2$~mrad'da tilt katkısı \emph{sıfırla tutarlıdır} ($-0.004\pm0.79$~nrad/s)
ve toplam sahte-EDM baseline'da ($\sim$1.3~nrad/s, $\beta$-beating-sınırlı) kalır;
tilt ancak $\psi\!\approx\!0.5$~mrad'da baskın olur ($+2.7$~nrad/s)
(Şekil~\ref{fig:syst}).}

\begin{figure*}[t]
\centering
\includegraphics[width=0.86\textwidth]{fig_kmod_syst.png}
\caption{Quad-tilt sistematiği (her quad bağımsız rastgele roll). (a) 0.3~$\mu$m
kalan misalignment üstünde tilt $\psi$-taraması (6 tohum): $\psi=0.2$~mrad'da
kalan sahte-EDM hedefte kalır ($\sim$1.3~nrad/s; tilt katkısı $\approx0$), yeşil
bant tolerans bölgesidir; $\psi=0.5$~mrad'da tilt baskın olur. (b) Tilt'in
CW/CCW EDM-kanalına katkısının söndürülebilir (EVEN) ve kalan (ODD) bileşenleri
(20 tohum, 10~$\mu$m + 1~mrad); ODD$>$EVEN olduğundan demet-yönü ters-çevirmesi
tilt'i \emph{gidermez}. Üretim: \texttt{make\_kmod\_figures.py}.}
\label{fig:syst}
\end{figure*}
...
Quad tilt (roll) & $\psi \lesssim 0.3$~mrad (0.2 doğrulandı) & mekanik / skew-BBA \\
...
nicellemez. Bizim özgün katkımız tam da bu \emph{köprüdür}: ``hangi
hizalama kipi sahte-EDM'i sürüyor'' ($\sigma^2$ mekanizması §\ref{sec:mechanism};
...
\%0.5$--$1$ \emph{ve} quad tilt $\psi \lesssim 0.3$~mrad düzeyinde
