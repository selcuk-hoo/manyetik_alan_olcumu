...
\title{Single-orbit reconstruction of quadrupole misalignments in a
frozen-spin storage ring: resolving a weak low-order harmonic in the
presence of large beam-position-monitor offsets}
...
extracting the low-order Fourier harmonic of the misalignment pattern
that dominates the false EDM ($k=2$ for our lattice), a single closed-orbit
...
motivated question. The false EDM is not driven equally by all 48 degrees
of freedom of $\Delta q$; it is dominated by the low-order Fourier
harmonics of the misalignment pattern, and for our lattice the leading
contribution is the $k=2$ harmonic---a fact we confirm directly by spin
tracking in Sec.~\ref{sec:falseedm}, consistent with the $N=2$ peak of
Omarov \emph{et al.}~\cite{omarov}. We therefore target $k=2$ directly
and show that it can be extracted from a \emph{single} orbit measurement
governed by Eq.~(\ref{eq:model}), keeping $\mathbf{b}$ in the model rather
than cancelling it. The key realisation is that the fitting-matrix column norm $\|M_{k=2}\|=167$
...
vertical tune $Q_y\approx2.68$). This reproduces---by an independent
mechanism (quadrupole misalignment rather than an imposed field harmonic)---the
$N=2$ peak reported by Omarov \emph{et al.}~\cite{omarov} (their
Figs.~7--8). There the low-harmonic rates were quoted only as statistical
upper limits, because the exact precession rate could not be determined from
the simulation; this is precisely the oscillation-contamination problem that
the closed-orbit\,/\,stroboscopic method removes here, returning the actual
rates.
...
For $k=2$ it outperforms the best kmod variant tested ($0.6\%$ vs.\ $\sim43\%$
amplitude error) while requiring only one orbit measurement and no gradient
modulation.
...
so in principle kmod achieves the same conditioning. In practice, 48
simultaneous precision gradient steps are operationally demanding;
the single-orbit method requires no active manipulation.
