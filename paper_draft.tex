...
\subsection{Monte Carlo systematic error budget}
\label{sec:systematics}

Four instrumental imperfections are quantified via Monte Carlo simulation
($N=200$ realizations each) using the CLEAN reconstruction with candidate set
$k=1\ldots10$.  The ``truth'' misalignment contains $k=1$ (30\,$\mu$m),
$k=2$ (10\,$\mu$m), $k=3$ (25\,$\mu$m) plus seven random contaminants
($k=4\ldots10$, 100--300\,$\mu$m each).

\paragraph{BPM readout noise.}
Independent Gaussian noise $\varepsilon_i\sim\mathcal{N}(0,\sigma^2)$ is
added to each BPM reading.  At $\sigma=5\,\mu$m, the $k=2$ amplitude
uncertainty is $\pm0.32\%$; at $\sigma=50\,\mu$m it grows to $\pm5.2\%$,
consistent with the analytic scaling $\sigma(\Delta A/A)\propto\sigma/\|M_k\|$.

\paragraph{BPM static offsets.}
A fixed per-BPM bias $b_i\sim\mathcal{N}(0,\sigma_b^2)$ shifts every orbit
reading but is invisible to the orbit fit (it bypasses $R$).  The analytic
prediction $\sigma(\Delta A/A)\approx\sigma_b/\|M_k\|$ is confirmed by
simulation: at $\sigma_b=50\,\mu$m the $k=2$ r.m.s.\ error is $0.4\%$;
at $\sigma_b=200\,\mu$m it reaches $1.5\%$, still well within the
\SI{10}{\micro\meter} target amplitude tolerance.

\paragraph{Quadrupole roll.}
A rolled quadrupole (rotation angle $\theta$ about the beam axis) couples the
horizontal closed orbit $x_\mathrm{co}$ into the vertical plane via skew
quadrupole terms: $\delta\mathrm{COD}\approx R_{dy}\cdot(2\theta\cdot x_\mathrm{co})$,
where $x_\mathrm{co}=R_{dx}\cdot\delta x$ and $\sigma_{dx}=100\,\mu$m.
At $\theta_\mathrm{rms}=1\,\mathrm{mrad}$, the $k=2$ amplitude error is
$-0.66\pm2.1\%$; the systematic bias remains $<1\%$ for all tested roll angles
up to 5\,mrad.

\paragraph{Quadrupole gradient errors.}
Per-element fractional gradient errors $\varepsilon_j\sim\mathcal{N}(0,\sigma_G^2)$
perturb the response matrix column-wise:
$y_\mathrm{meas}\approx y_\mathrm{nom}+R\cdot(\varepsilon\odot\Delta y_\mathrm{truth})$.
At $\sigma_G=0.5\%$ the $k=2$ error is $-0.84\pm3.9\%$; at $\sigma_G=2\%$
it reaches $\pm16\%$, making tight gradient calibration important.

\paragraph{Combined budget.}
With all four effects applied simultaneously at their acceptance levels
($\sigma_\mathrm{noise}=5\,\mu$m, $\sigma_b=50\,\mu$m, $\theta_\mathrm{rms}=1\,\mathrm{mrad}$,
$\sigma_G=0.5\%$), the $k=2$ amplitude error is $-0.1\pm5.4\%$, dominated
by the gradient uncertainty.  The target of $\lesssim10\%$ relative amplitude
accuracy is met.  Tighter gradient calibration ($\sigma_G\lesssim0.2\%$) would
reduce the combined uncertainty below $2\%$.

\begin{table}[h]
\centering
\caption{Systematic error budget at acceptance levels (CLEAN, $N=200$ MC).
$\mu$ and $\sigma$ are the mean and standard deviation of $\Delta A/A$ [\%]
over the Monte Carlo ensemble.}
\label{tab:systematics}
\begin{tabular}{lccccc}
\toprule
Source & Level & $k=1$ ($\mu\pm\sigma$) & $k=2$ ($\mu\pm\sigma$) & $k=3$ ($\mu\pm\sigma$) \\
\midrule
BPM noise    & 5\,$\mu$m  & $-0.93\pm0.38$ & $-0.74\pm0.32$ & $-0.92\pm0.49$ \\
BPM offset   & 50\,$\mu$m & $-0.80\pm3.98$ & $-0.13\pm2.51$ & $-0.66\pm4.69$ \\
Quad roll    & 1\,mrad    & $-0.89\pm0.62$ & $-0.62\pm1.92$ & $-0.95\pm0.72$ \\
Gradient err & 0.5\%      & $-0.90\pm1.45$ & $-0.84\pm3.94$ & $-1.15\pm1.71$ \\
\midrule
Combined     & all above  & $-0.83\pm4.38$ & $-0.12\pm5.37$ & $-0.69\pm5.21$ \\
\bottomrule
\end{tabular}
\end{table}
