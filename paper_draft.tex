...
vertical tune $Q_y\approx2.68$). The rate falls monotonically for $k\ge3$,
reaching $4\%$ of the $k=2$ peak at $k=6{,}7$. This is qualitatively consistent with the
...
6 & 0.008 & $-5.92\times10^{-11}$ & 0.041 \\
7 & 0.005 & $-4.73\times10^{-11}$ & 0.033 \\
\bottomrule
\end{tabular}
\end{table}
...
= \frac{\SI{10}{\micro\meter}\times167}{\SI{300}{\micro\meter}}
\approx 5.6 .
\end{equation}
...
\subsection{Spin-equivalent external radial field}
\label{sec:bxequiv}

The misalignment picture can be translated into an equivalent external field
language that connects directly to Omarov \emph{et al.}~\cite{omarov}.
Define the spin-equivalent radial field $B_{x,\mathrm{eq}}^{(k)}$ as the
amplitude of a ring-wide harmonic $B_x(\theta)=B_{x,\mathrm{eq}}\cos(k\theta)$
that would produce the same $dS_y/dt$ as the $k$-th misalignment harmonic.
From two independent spin-tracking scans---one measuring $R_B(k)=[dS_y/dt]/A_r$
for external fields (compare\_field\_harmonics), the other measuring
$R_q(k)=[dS_y/dt]/A_q$ for quad misalignments (false\_edm\_mode\_scan)---we
form the conversion factor
\begin{equation}
c_k = \frac{R_q(k)}{R_B(k)} \quad [\mathrm{nT}\,\mu\mathrm{m}^{-1}].
\label{eq:ck}
\end{equation}
Numerically, $c_k \approx 6\,\mathrm{nT}\,\mu\mathrm{m}^{-1}$ for $k=1,\ldots,7$,
to within $\pm17\%$.  This near-independence of $k$ is physically natural: the
spin couples to the ring-average radial impulse rather than the local
peak, and the FODO fill factor $\sim3\%$ sets the overall scale
($c_k \approx G\,\ell_\mathrm{eff}\,f_\mathrm{fill}/L_\mathrm{ring}$ with
$G=0.21\,\mathrm{T/m}$). The local quad-centre field is therefore
$\sim34\times$ larger than the spin-equivalent value, making spin tracking
the correct reference.

For the test signal modes ($k=2$: \SI{10}{\micro\meter}; $k=3$: \SI{8}{\micro\meter}),
the spin-equivalent amplitudes are
$c_2\times\SI{10}{\micro\meter}\approx\SI{54}{\nano\tesla}$ and
$c_3\times\SI{8}{\micro\meter}\approx\SI{46}{\nano\tesla}$, respectively.
...
Three BPM-offset levels ($\sigma_b = 0$, $100$, $300\,\mu$m) are overlaid:
...
even at the worst-case $\sigma_b=\SI{300}{\micro\meter}$,
...
\item \textbf{Signal orbit ($\SI{1670}{\micro\meter}$).}
  ...
  in BPM space.
\item \textbf{BPM-offset Fourier level ($\approx\SI{77}{\micro\meter}$).}
  Because $\mathbf{b}$ bypasses $R$ entirely and is white, its projection
  onto any FODO-antisymmetric mode $F_k$ has r.m.s.\
  $\sigma_b\sqrt{\pi/48}\approx\SI{77}{\micro\meter}$---distributed equally
  across all $k$, with no peak at $k=2$.
  The signal orbit ($\SI{1670}{\micro\meter}$) is already $22\times$ larger than
  this raw Fourier level.
\item \textbf{Estimator contamination floor ($\approx\SI{1.8}{\micro\meter}$).}
  The estimator projects $\mathbf{b}$ onto the $k=2$ column of $M=RF$
  (not onto $F_{k=2}$ directly), dividing by $\|M_{k=2}\|=167$ a second
  time: $\sigma_b/\|M_{k=2}\|=\SI{300}{\micro\meter}/167\approx\SI{1.8}{\micro\meter}$.
\end{enumerate}
The hierarchy
$\SI{1670}{\micro\meter}\gg\SI{77}{\micro\meter}\gg\SI{1.8}{\micro\meter}$
...
$\sigma_b=\SI{300}{\micro\meter}$ offset alongside a $\SI{10}{\micro\meter}$
$k=2$ signal, recovering $\hat{a}_{k=2}=10.0\,\mu$m and $1.3\,\mu$m respectively.
...
is lowest precisely at $k=2$ ($\SI{2.2}{\micro\meter}$) because $\|M_{k=2}\|=167$
