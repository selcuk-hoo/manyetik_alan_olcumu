...
$\Delta q = R^{-1}(\mathbf{y}-\mathbf{b})$ is impossible because
$\mathbf{b}$ is unknown, and $R^{-1}\mathbf{y}$ carries an
$R^{-1}\mathbf{b}$ contamination that, with $\kappa(R)\approx249$, reaches
the millimetre level---two orders of magnitude above the target.
...
contribution is the $k=2$ harmonic. We therefore target $k=2$ directly
...
than cancelling it. The key realisation is that $\mathbf{b}$ is white
across the BPMs whereas $R\,\Delta q$ is a structured pattern whose $k=2$
component is resonantly amplified by the lattice; the two are separable
by least squares. Section~\ref{sec:sim} describes the simulation,
...
CLEAN iteration borrowed from radio astronomy. With residual
...
commitment to a single mode and is robust against the residual
mode-mixing in $M$.
...
Each unit-norm Fourier mode is mapped to an orbit of norm $\|RF_k\|$.
Table~\ref{tab:gain} shows this ``gain'' peaks sharply at $k=2$, the mode
whose phase advance per cell ($2\pi\cdot2/48$) lies closest to the
fractional tune. The $k=2$ gain exceeds that of $k=4$ by $\times10$ and
of $k=8$ by $\times58$.

\begin{table}[t]
\centering
\caption{Orbit gain $\|RF_k\|$ of unit-norm Fourier modes. The tune
$Q\approx2.68$ places $k=2,3$ closest to resonance.}
\label{tab:gain}
...
misalignment produces a \SI{1669}{\micro\meter} orbit norm---comparable
to $k=6$ and larger than $k=8$---so $k=2$ is no longer a buried weak
signal but one of the leading orbit components.
...
Because $\mathbf{b}$ is white, it projects a small amount onto every
candidate mode; with the oracle-free candidate set $k=1,\dots,12$, CLEAN
flags all of them above the \SI{1}{\micro\meter} level, so the genuine
harmonics ($k=2,4,6,8$) cannot be distinguished from offset-induced
spurious ones on amplitude alone. This does not corrupt the $k=2$
\emph{amplitude} (the modes are orthogonal under $R$), but it means the
method, as it stands, measures a \emph{targeted} harmonic rather than
discovering the harmonic content. Discrimination strategies are discussed
...
For $k=2$ it outperforms the best kmod variant we tested
($0.6\%$ vs.\ $\sim43\%$ amplitude error) while requiring only one orbit
measurement and no gradient modulation. The following items remain open.

\begin{itemize}
\item \todo{Validate at the realistic mechanical offset
$\sigma_b=\SI{300}{\micro\meter}$ with full Monte-Carlo statistics and a
non-zero BPM readout noise $\eta$.}
...
response matrix: tune-proximity amplification of $k=2$, the consequent
inversion of the misalignment-amplitude hierarchy at the orbit level, and
the near-orthogonality of the harmonic orbit responses that prevents
contaminant leakage. We also documented and corrected a $10^3$
unit-scaling error in the response-matrix pipeline. The principal open
problem is discriminating genuine harmonics from offset-induced spurious
ones without prior knowledge; the items in Section~\ref{sec:future} define
the path toward a validation at realistic offsets and a spin-level error
budget.
...
Phys.\ Rev.\ D \textbf{105} (2022) 032001. \todo{verify reference}
