...
**Test SONUCU (KOŞULDU → kavramsal POZİTİF ama pratik ZAMAN-YASAK, `akilli_duzeltme.md
§6.12`):** en-kötü orbit-kör simetrik makine (COD 1.6 μm); gerçek(C++) gradyan
Gauss-Newton. Gürültüsüz: orbit-null işe yaramadı (1.0×); SPİN-descent 2 adımda
hedefin altına indi (726×). **AMA (kullanıcı itirazı) gürültüsüz demo ücretsiz-ölçüm
varsayar; gerçekte polarimetre İSTATİSTİKLE SINIRLI:** dS_y/dt'yi EDM-seviyesine
(~1 nrad/s) ölçmek ~**50 yıl** (`cosy_polarimeter §4`; 1 yıl→7× hedef, σ∝1/√T).
Descent ~40–100 ölçüm ister → **onyıllar-yüzyıllar → iteratif spin-null İMKÂNSIZ.**
Aynı sınır §14.6 spin-trim için de geçerli.

> **ASIL BAĞ (bind):** orbit-tabanlı kontrol **hızlı ama tıkalı** (simetrik-kör);
> spin-tabanlı **doğru ama istatistik-yasak** (yıllar/ölçüm). İki kaçış da kapalı →
> simetrik sistematik gerçekten zor. Omarov iteratif spin-null yapmaz: CW/CCW iptal +
> a-priori orbit (CR-ayrım, hızlı, ama simetriğe kör §9.3) + kalanı tasarım toleransıyla
> (μm→~62× hedef) sınırlar. **Aktif null yerine a-priori sınırlama** gerçek deneyin yolu.
...
| **5** spin-modülasyon descent | kavramsal çalışır (726×) AMA **istatistik-yasak** (yıllar/ölçüm → iteratif imkânsız) | — | ∞ (zaman) | **✓ KOŞULDU: kavramsal-pozitif/pratik-YASAK** |
