...
\label{sec:ring}
This study is made with the same symmetric-hybrid ring of Ref.~\cite{omarov}: composed of 24 FODO cells with electric  deflectors and magnetic quadrupoles. The focusing (QF) and defocusing (QD) quadrupoles have the same gradient ($g=0.1$ T/m) with opposite signs, separated by drift spaces and deflectors:
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
The deflectors are basically cylindrical capacitors of 95.5 m radius. To store the beam at magic momentum, the electric field is set to $E_0=4.4$ MV/m. Some of the ring and beam parameters are given at
\begin{table}[]
\centering
...
The fundamental question of this study is whether standard orbit correction (OC) methods can be useful. It typically relies on singular value decomposition (SVD) of the response matrix (R) and minimizing the closed orbit distortion. This approach is applied routinely since Chung, Decker and Evans~\cite{chung1993} and can work continuously and seamlessly during data taking. The closed orbit is obtained by the beam position monitors (BPM) next to each quadrupole. Response matrix regularization~\cite{ziemann}, symmetry correction~\cite{mirza} and orbit response modelling techniques~\cite{wegscheider} can be utilized as different treatments to the response matrix.

It turns out that regardless of which technique is being used, simple orbit smoothing is not sufficient to eliminate this geometric phase effect. Because the mechanisms for false EDM generation and closed orbit distortion are fundamentally different. However, OC with CR beams together is useful for fixing the false EDM signal. This measurement still cannot replace the original proposal of Ref.~\cite{omarov} because BPM offset spoils it. However, it can be useful for drift monitoring of the quadrupoles, once they are calibrated by BBA.

\subsection{Contribution of this work}
\label{sec:contribution}
The main contribution of this study is the redefinition of the relationship between the closed orbit and the geometric phase effect. The false EDM signal is not only defined by the RMS of the quadrupole misalignments, but also their pattern. Simple orbit smoothing can hardly fix the \textit{symmetric mode} of the misalignments, so it is not sufficient for the elimination of the false EDM. Closed orbits of the CR beams as summed reveal this symmetric mode, which allows continuous monitoring after first calibration. Below is the short summary.
...
The genuine EDM as defined through the coupling parameter $\eta = 1.88 \times 10^{-15}$, corresponding to $d_p\approx 10^{-29}~e\cdot$cm, yielded $dS_y/dt = 9.81 \times 10^{-10}$ rad/s in the simulations. This value was inversed for the CR particle, consistent with Ref.~\cite{omarov}. All the false EDM estimations were made by artificially setting $\eta=0$, so that the false EDM signal was isolated. Gradient errors, quadrupole misalignments and tilts, and the other studied systematic error were introduced as ring parameters.
...
\begin{enumerate}
    ...
    \item Inversing the sign of the misalignments yielded
...
It is worth noting that BBA does not require inversing the response matrix. Therefore, it is more immune against the discrepancy between the symmetric and antisymmetric modes. It becomes a significant difficulty for SVD algorithms (See Figure \ref{fig:fig_orbit_modes}).
...
BBA with CR beams, followed by quadrupole polarity switch at consecutive runs is robust in suppressing the both components, hence the false EDM signal. The symmetric mode is buried under noise and other realistic systematic effects in conventional orbit correction methods. However, using summation of the closed orbits of the counter-rotating beams extract the symmetric component, eventually leading to a faster suppression of the orbit and the false EDM. It brings an additional advantage -- no requirement for quadrupole polarity switch. Hence, the experimental time reduces by half for the same sensitivity.
