...
| **⚠️ YENİ AÇILIM (2026-07): null-arayan klasik BBA — "hiçbir orbit yöntemi ölçemez" hükmüne AYRI SINIF istisnası. Genlik-okuma (nefes) ve inversiyon (koşullanma) sınıflarından farklı: quad gradyan-mod + demet tarama + sıfır-geçişi. TEMİZ OPTİKTE simetriği ÖLÇÜYOR (356×→1.6× hedef, C++). β-beat'te ham yörüngede çöküyor (nefes), iterasyonla DİKEY kurtarılıyor (0.14μm), YATAY açık (dikiş-quad'ları). Donanım tabanı (mod-altı merkez hareketi) simüle edilmedi. AÇIK+ÜMİT VERİCİ, kapalı no-go DEĞİL** | `separation_bba_testleri.md §5`, `why_bba_works.md`, `bba_betikleri.md` |
